  ...
  <AppLayout>
    ...
    >
      ...
      <form @submit.prevent="submit(route().current())">
        <div class="flex sm:space-x-3 sm:flex-row flex-col">
          <div class="mb-3 w-full">
            <label class="block text-gray-500 text-sm font-bold mb-2" for="kode_matakuliah">
              Kode Matakuliah
            </label>
            <Input
          ...
          <div class="mb-3 w-full">
            <label class="block text-gray-500 text-sm font-bold mb-2" for="nama_matakuliah">
              Nama Matakuliah
            </label>
            <Input
        ...
        <div class="flex sm:space-x-3 sm:flex-row flex-col">
          ...
          <div class="mb-3 w-full">
            <label class="block text-gray-500 text-sm font-bold mb-2" for="nilai">
              Nilai
            </label>
            <Input
              v-model="form.nilai"
        ...
        <div class="mb-3">
          <label class="block text-gray-500 text-sm font-bold mb-2" for="matakuliah">
            Matakuliah yang Setara
          </label>
          <select 
            class="w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm" 
            id="matakuliah"
            v-model="form.matakuliah_baru_id">
            <option v-for="matakuliah in matakuliahs" :key="matakuliah.id" :value="matakuliah.id.toString()">{{matakuliah.nama_matakuliah}} - {{matakuliah.kode_matakuliah}} - {{matakuliah.kurikulum.nama}}</option>
          </select>
        </div>
      ...
      <div class="flex justify-between">
        <div>
          <Button @click="submit(route().current())" class="px-10">Simpan</Button>
        </div>
        <Link
